...
// selfie bootstraps void* to uint_t* and unsigned to uint_t!
...
void reset_profiler() {
  ...
  iterations          = 0;
  iterations_per_loop = zalloc(code_length / INSTRUCTIONSIZE * SIZEOFUINT);
...
uint_t* create_elf32_header(uint_t binary_length, uint_t code_length) {
  ...
  *(header + 1)  = 1                                 // file class is ELFCLASS32
                 + left_shift(1, 8)                  // object file data structures endianness is ELFDATA2LSB
